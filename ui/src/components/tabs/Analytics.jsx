...
export default function Analytics({ services, hourly }) {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      {spikes.length > 0 && (
        <div style={{ ...CARD, background: "#fef2f2", borderColor: "#fecaca" }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {spikes.map((s) => (
              <div key={s.service_name} style={{ background: "#fff", border: "1px solid #fca5a5", borderRadius: 8, padding: "8px 14px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(185px, 1fr))", gap: 10 }}>
        {services.map((s) => {
          ...
          const rb = s.error_rate > 10 ? "#fef2f2" : s.error_rate > 3 ? "#fffbeb" : "#f0fdf4";
          return (
            <div key={s.service_name} style={CARD}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                <span style={{ fontSize: 11, color: "#111827", fontWeight: 600 }}>{s.service_name}</span>
              ...
              <div style={{ display: "flex", alignItems: "flex-end", gap: 2, height: 24, marginBottom: 10 }}>
                {Array.from({ length: 14 }, (_, i) => {
                  const h = rnd(2, 20);
                  return <div key={i} style={{ flex: 1, height: h, borderRadius: 2, background: h > 15 ? "#fca5a5" : "#e5e7eb" }} />;
                })}
              </div>
              <div style={{ fontSize: 10, color: "#6b7280", display: "flex", gap: 12 }}>
                <span><span style={{ color: "#111827", fontWeight: 500 }}>{s.total_events?.toLocaleString()}</span> events</span>
      ...
      {/* Hourly chart */}
      <div style={CARD}>
        <div style={{ fontSize: 10, color: "#6b7280", letterSpacing: 1, marginBottom: 16, fontWeight: 500 }}>HOURLY VOLUME</div>
        <ResponsiveContainer width="100%" height={200}>
          <BarChart data={hourly} barSize={7} margin={{ left: -20, right: 8 }}>
            ...
            <Tooltip
              contentStyle={{ background: "#fff", border: "1px solid #e5e7eb", fontSize: 11, borderRadius: 8, fontFamily: "'IBM Plex Mono', monospace", boxShadow: "0 4px 12px rgba(0,0,0,0.08)" }}
              labelStyle={{ color: "#374151" }} itemStyle={{ color: "#6b7280" }}
            />
            {SEVERITIES.map((s) => (
        ...
        <div style={{ display: "flex", gap: 16, justifyContent: "center", marginTop: 8 }}>
          {SEVERITIES.map((s) => (
            <div key={s} style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 9, color: "#6b7280" }}>
