...
function ArchDiagram({ theme }) {
  ...
  export default function SystemTab({ live, coldStart, theme, t, onOpenGenerator }) {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      {/* TOP ROW — diagram left, stats right */}
      <div style={{
        display: "grid",
      ...
      {/* BOTTOM ROW — free tier note full width */}
      <div style={{
        ...CARD,
        ...
        <div>
          <div style={{ fontSize: 10, color: "#92400e", letterSpacing: 1, marginBottom: 6, fontWeight: 600 }}>
            FREE TIER NOTE
          </div>
          <div style={{ fontSize: 11, color: dk ? "#d97706" : "#78350f", lineHeight: 1.8 }}>
            Fly.io machines sleep after ~5 min of inactivity. Cold starts take 5–15 seconds — the boot sequence handles this automatically.
          </div>
        </div>
        <div style={{ display: "flex", gap: 32, flexWrap: "wrap" }}>
          {[
            ["Fly.io",    "Free · auto-sleep"],
            ["Astra DB",  "Always-on · free tier"],
            ["Neon",      "Always-on · free tier"],
            ["Vercel",    "Free · CDN"],
          ].map(([service, note]) => (
            <div key={service}>
              <div style={{ fontSize: 10, color: "#92400e", fontWeight: 600, marginBottom: 2 }}>{service}</div>
              <div style={{ fontSize: 10, color: dk ? "#d97706" : "#78350f" }}>{note}</div>
            </div>
          ))}
        </div>
      </div>
